...
from metrics.bert_classifier import get_optimizer
...
def fine_tune_lm(output_dir, trainset, filter, device, lm_steps=5000, lm_bs=32,
    ...
    if filter == -1:
        lm_model = BertForMaskedLM.from_pretrained(utils.get_transformers(model_init))
        lm_model.train()
    else:
...
def non_autoregressive_fine_tune_lm(
    ...
    if filter == -1:
        lm_model = NonAutoregressiveBertLM.from_pretrained(utils.get_transformers(model_init),
