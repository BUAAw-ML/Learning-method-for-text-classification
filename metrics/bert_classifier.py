...
from metrics.classifier_base import ClassifierBase
...
class BertClassifier(ClassifierBase):
    ...
    def __init__(self, dataset_name, trainset, testset, bert_gpu_id=-1,
                 ...
                 initial_model_with_trainset=False, **kargs):
        ...
        if initial_model_with_trainset: 
            ...
            utils.get_transformers(model_init), num_labels=len(trainset["label_mapping"])).to(self._device)
    ...
    def evaluate_classifier(self, eval_bs):
        
        dataloader_val = torch.utils.data.DataLoader(
            DatasetForBert(self._testset, self._model_init, eval_bs), batch_size=None, num_workers=1)
        dataloader_val_iter = iter(dataloader_val)
        ...
        for v_step in tqdm.tqdm(range(math.ceil(len(self._testset["data"]) / eval_bs)), total=math.ceil(len(self._testset["data"]) / eval_bs)):
            seq, mask, tok_type, label = next(dataloader_val_iter)
            seq = seq.to(self._device)
            mask = mask.to(self._device)
            tok_type = tok_type.to(self._device)
            label = label.to(self._device)
            
            with torch.no_grad():
                outputs = self._model(seq, mask, tok_type, labels=label)
                loss, logits = outputs[:2]
                loss_list.append(loss.detach().cpu().numpy())
                count += seq.size(0)
                correct += (logits.argmax(dim=1).eq(label)
        ...
        acc = correct / count
        print("clf_val/acc", acc)
        print("clf_val/loss", np.mean(loss_list))
