...
from utils.download_utils import download_file, get_root_dir
from datasets.downloadable_datasets import downloadable_dataset_urls
...
class DatasetForBert(torch.utils.data.IterableDataset): 
    ...
    def __init__(self, dataset, model_init, batch_size, label_num=54, exclude=-1,
                 masked_lm=False, masked_lm_ratio=0.2, dynamic_masked_lm=False,
                 include_raw_text=False, seed=0, clf_type="multi_label_classify"):
        """Initialize.
    ...
    def __iter__(self):
        ...
        while True:
            ...
            if not self._masked_lm and not self._dynamic_masked_lm:
                ret = [torch.tensor(x) for x in [texts, masks, tok_types]]
                if self._clf_type == "multi_label_classify":
                    labels_matrix = torch.zeros(size=(self._batch_size, self._label_num))
                    for i in range(self._batch_size):
                        labels_matrix[i, labels[i]] = 1.
                    ret = [tuple(ret), labels_matrix]
                elif self._clf_type == "multi_classify":
                    ret.append(labels)
                ...
                ret = [torch.tensor(x) for x in [texts, masks, tok_types, lm_labels]]#labels, 
