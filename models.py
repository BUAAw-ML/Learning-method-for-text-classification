...
class MABert(nn.Module):
    ...
    def forward(self, ids, token_type_ids, attention_mask, encoded_tag, tag_mask, feat):
        ...
        print(torch.sum(hidden_out_fake[:,-1] * self.class_weight, -1).shape)
        attention_out_fake = torch.cat((attention_out_fake, torch.sum(hidden_out_fake[:,-1] * self.class_weight, -1)), -1)
