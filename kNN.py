...
def getMaxDist(dg, n):
    if not dg.nodes()[n]['neibs']:
        return None
    k = max(dg.nodes()[n]['neibs'], key=dg.nodes()[n]['neibs'].get)
    d = dg.nodes()[n]['neibs'][k]
    return (k,d)
...
def NNDescentFullReworked (dg, k, p, stoprate):
    for i in dg.nodes():
        ...
        other_nodes = list(dg.nodes())[:(i - 1)] + list(dg.nodes())[i:]

        s = sample(other_nodes, k)
        for j in s:
            dg.nodes()[j]['rev_neibs'].append(i)
            dg.nodes()[i]['neibs'][j] = dist(dg.nodes()[j], dg.nodes()[i])
    while True:
        updates = 0
        for i in dg.nodes():
            seed()
            old = [n for n in dg.nodes()[i]['neibs'] if dg.nodes()[n]['isnew'] is False]
            new = [n for n in dg.nodes()[i]['neibs'] if dg.nodes()[n]['isnew'] is True]
            new = sample(new, int(len(new)*p))
            for n in new:
                dg.nodes()[n]['isnew'] = False
            old_rev = []
            new_rev = []
            for n in old:
                old_rev.extend(dg.nodes()[n]['rev_neibs'])
            for n in new:
                new_rev.extend(dg.nodes()[n]['rev_neibs'])
        ...
        if updates < stoprate*k*len(dg.nodes()):
            for n1 in dg.nodes():
                for n2 in dg.nodes()[n1]['neibs']:
                    dg.add_edge(n1, n2)
            return


def updateNeighborsReworked (dg, head, tail):
    if tail in dg.nodes()[head]['neibs']:
        return 0
    maxD = getMaxDist(dg, head)
    d = dist(dg.nodes()[head], dg.nodes()[tail])
    if maxD[1] > d:
        del dg.nodes()[head]['neibs'][maxD[0]]
        dg.nodes()[maxD[0]]['rev_neibs'].remove(head)
        dg.nodes()[head]['neibs'][tail] = d
        dg.nodes()[tail]['rev_neibs'].append(head)
        return 1
    else:
        return 0
...
def evaluateRecall (dg, K):
    ...
    for i in dg.nodes():
        ...
        for j in dg.nodes():
            if i != j:
                distances[j] = dist(dg.nodes()[i], dg.nodes()[j])
        sort_by_dist = sorted(distances, key=distances.get)
        for j in sort_by_dist[0:K]:
            if j in dg.nodes()[i]['neibs'] or distances[j] == getMaxDist(dg,i)[1]:
                rights += 1
            else:
                wrongs += 1
...
for j in [1000, 1250, 1500, 1750, 2000, 2500, 3000, 3500, 4000, 4500, 5000, 5500, 6000, 6500, 7000, 8000, 9000]:
    ...
    pos = {}
    labels = {}
    f = open("worldcitiespop.txt",'r',encoding="ISO-8859-1")
    ...
    nx.draw(G, pos = nx.random_layout(G),  node_color = 'b',edge_color = 'r', node_size=30)
